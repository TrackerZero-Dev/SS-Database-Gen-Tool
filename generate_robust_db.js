...
try {
  ...
  const decryptedFiles = fs
    .readdirSync(DIRS.decrypted)
    .filter((f) => f.endsWith(".json"));
  if (decryptedFiles.length === 0) {
    console.error(
      "[System] No data files found in database/decrypted/ or database/encrypted/.",
    );
    console.error(
      "         Please place your raw game data file there and run again.",
    );
    process.exit(1);
  }
  ...
  for (const file of decryptedFiles) {
    try {
      const data = JSON.parse(
        fs.readFileSync(path.join(DIRS.decrypted, file), "utf-8"),
      );
  ...
  if (Object.keys(metadata).length > 0) {
    ...
    fs.writeFileSync(
      metadataPath,
      JSON.stringify(unifiedMetadata, null, 2),
      "utf-8",
    );
    console.log(
